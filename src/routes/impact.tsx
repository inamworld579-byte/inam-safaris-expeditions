...
export const Route = createFileRoute("/impact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Conservation, community uplift and ethical travel — how INAM gives back to the hills, the gorillas and the people of Rwanda." },
    ],
  ...
  return (
    <div>
      <section className="relative h-[70vh] overflow-hidden">
        <img src={heroRwandaImg} alt="People in Rwanda" className="absolute inset-0 w-full h-full object-cover ken-burns" />
        ...
        <div className="relative h-full flex flex-col justify-end pb-20 px-6 md:px-16 max-w-[1400px] mx-auto text-cream">
          <Reveal><div className="eyebrow text-kaki mb-4">Impact</div></Reveal>
          <Reveal delay={0.1}>
      ...
      <section className="py-24 px-6 md:px-16 max-w-[1400px] mx-auto">
        <Reveal>
          <p className="font-display text-2xl md:text-4xl text-forest max-w-4xl leading-snug">
            INAM stands for "It's Never About Me". It's not a slogan — it's an accounting principle. A portion of every booking funds conservation, community and craft.
          </p>
        </Reveal>
      ...
      <section className="bg-cream py-24 px-6 md:px-16">
        <div className="max-w-[1400px] mx-auto grid md:grid-cols-3 gap-10">
          {[
            { icon: Shield, t: "Gorilla Habitat Protection", b: "We partner with rangers in Volcanoes NP to fund anti-poaching patrols and forest restoration along the Virunga corridor." },
            { icon: Users, t: "Women's Coffee Cooperatives", b: "Our coffee partners are 4 women-led cooperatives across the western volcanic slopes." },
      ...
      <section className="py-24 px-6 md:px-16 max-w-[1400px] mx-auto grid md:grid-cols-2 gap-10 items-center">
        <Reveal>
          <img src={coffeePound} alt="Coffee processing" className="aspect-[4/5] w-full object-cover" />
        </Reveal>
        <Reveal delay={0.2}>
          <div>
            <div className="eyebrow text-chocolate mb-3">A Story from the Hills</div>
            <h2 className="font-display text-4xl md:text-5xl text-forest leading-tight mb-6">"My daughters now study in Musanze because of the coffee."</h2>
            <p className="text-stone-warm text-lg leading-relaxed">
               Mukamana Beatrice, founder of one of our partner cooperatives in the Virunga foothills. Her cooperative employs 38 women and exports two tonnes of speciality coffee a year.
            </p>
          </div>
        </Reveal>
      </section>
